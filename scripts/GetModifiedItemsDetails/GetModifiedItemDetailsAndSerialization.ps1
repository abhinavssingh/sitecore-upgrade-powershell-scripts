...
function Get-ModifiedItems {
    ...
    try {
        if (![string]::IsNullOrEmpty($ItemPath)) {           
            if ( $IncludeNested ) {
                $parent = Get-Item -Path "master:$ItemPath" | Where-Object { $ExcludeTemplates -notcontains $_.TemplateId } | Where-Object { ($_.__Updated).Date -ge $startDate -and ($_.__Updated).Date -lt $endDate } 
                $parent
                Export-Item -path master:$ItemPath
                $children = Get-ChildItem -Path "master:$ItemPath" -Recurse | Where-Object { $ExcludeTemplates -notcontains $_.TemplateId } | Where-Object { ($_.__Updated).Date -ge $startDate -and ($_.__Updated).Date -lt $endDate } 
                ...
                $item = Get-Item -Path "master:$ItemPath" | Where-Object { $ExcludeTemplates -notcontains $_.TemplateId } | Where-Object { ($_.__Updated).Date -ge $startDate -and ($_.__Updated).Date -lt $endDate } 
                $item
                Export-Item -path master:$ItemPath
            }
        }
