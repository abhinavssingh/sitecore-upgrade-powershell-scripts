...
$serializedFolderPath = "\App_Data\serialization\security\sitecore\Roles\{0}.role"
...
if ($null -eq $csvData) {
        ...
        foreach ($row in $csvData) {
                $serializedPath = $serializedPath -f $row.RoleName      
                Import-Role -Path $serializedPath
        }
}
